...
function findScoreIndex(leaderboardText, scoreAttribute) {
	var scoreIndex = -1;
	var i;
	var studentId = parent.window.gameBridge
		? parent.window.gameBridge.info.studentId
		: null;
...
function initRetryLoaders(game, context, cb) {
	...
	game.load.onLoadComplete.add(
		loadComplete.bind(context, cb, retries),
		context
	);
}
...
function fileComplete(progress, cacheKey, success, totalLoaded, totalFiles) {
	...
	// can load up to 99%
	if (!initialized) {
		updateParentLoader(INITIAL_LOAD_PERCENT - 1 + progress * 0.9);
	}
}
